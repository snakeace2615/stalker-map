    ...
    public selectItem(item: Item): void {
        ...
        if (isWeapon(item)) {
            ...
            if (uniqueDamage.length == 1) {
                ...
                let shotsPerSec: number = item.rpm / 60;

                if (shotsPerSec > item.ammoMagazineSize) {
                    shotsPerSec = item.ammoMagazineSize;
                }

                damage.valuePerSecound = this.Math.floor(shotsPerSec * damage.value);
                damage.valuePerMag = this.Math.floor(item.ammoMagazineSize * damage.value);
